...
plot_tx_expression <- function(mtr_stats_wider_all, gene_name){
    ...
    # print(head(mtr_stats_wider_all))
    # stop()

	p <- ggplot(mtr_stats_wider_all, aes(x=factor(time) )) #color = str_wrap(rowname, 10)		
	
	p <- p + geom_line( aes(y=mean , group = ct, color = ct), linewidth = 0.7) +
				...
				geom_errorbar(aes(ymin=mean - sd, color = ct, ymax=mean + sd),
	                  					position=position_dodge(0.05), width=.2 , alpha = 0.5) +
				scale_color_manual(values=c("#CC6666", "#9999CC","#348540", "#841bb8"))
	...
	p <- p + theme_bw() +
  	# scale_y_continuous(breaks = scales::breaks_pretty(n = 0)) +
		...
		labs(x = "", colour = NULL,			#Timepoints			
				y = "Mean Normalized Expression \n(Log2 TPM)", 
				title = paste0(gene_name, "")) +    # : miRNA expression")) +
...
for (ct in head(cltyps, 4)) {
    ...
    cat("reading from file: ", paste0(dir,
                                        ct,
                                        "/", ct,
                                        filename))
    gene_expr <- read.csv2(paste0(dir,
                                ct,
                                "/", ct,
                                filename),
					header = TRUE, sep = "\t") %>% 
                    filter(str_detect(miRNA, paste0((opt$ens), "\\b")))            # %>% filter(str_detect(X, paste0(opt$ens, '.'))) 
