...
print("hello: Running miRNA differential expression analysis for HC samples")
paste("Current working directory:", getwd())

# samples_HC <- read.table(file.path("HC/samples_HC_new.txt"), header=TRUE)
# rownames(samples_HC) = c(paste("ctrl", rep("HC",4), 1:4, sep="_"),
...
# data.mir.HC = read.table(file.path("HC/HC_mature_normalized_CPM.txt"), header=TRUE)
...
# fitHC$i # returns the number of significant genes
...
#          show_rownames=F,
#          border_color=NA,
#          filename="Heatmap_significant_miR_HC_mod.png"
# )
...
# write.table(get$summary, "HC_summary_miRNAs.txt", na = "NA", quote=FALSE, sep=",")
...
samples_HC <- read.table(file.path("HC/samples_HC_remout.txt"), header=TRUE)
