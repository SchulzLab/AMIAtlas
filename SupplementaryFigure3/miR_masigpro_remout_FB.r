...
library(pheatmap)
#==================== remove the outlier samples and run again d0, rep 2 ==================
print("hello: Running miRNA differential expression analysis removing outlier FB samples")
samples_FB <- read.table(file.path("./configurations/samples_FB_remout.txt"), header=TRUE)
...
data.mir.FB = read.table(file.path("./expression/FB/FB_mature_normalized_CPM.txt"), header=TRUE)
