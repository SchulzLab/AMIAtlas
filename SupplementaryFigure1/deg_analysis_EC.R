...
samples_EC <- read.table(file.path("samples_EC.txt"), header=TRUE)
dir = getwd()
filesEC <- file.path(dir, samples_EC$pop, paste(samples_EC$run, "_transcripts_quant", sep=""), "quant.sf")

names(filesEC) = c(paste("d0", rep("EC",4), 1:4, sep="_"),
               paste("d1", rep("EC",4), 1:4, sep="_"),
               paste("d3", rep("EC",4), 1:4, sep="_"),
               # paste("d3", rep("EC",3), paste(4, ".", sep="", 1:3), sep="_"),
               paste("d7", rep("EC",4), 1:4, sep="_"),
               paste("d14", rep("EC",4), 1:4, sep="_"),
               # paste("d14", rep("EC",4), paste(1:4, ".", sep="", 2), sep="_"),
               paste("d28", rep("EC",4), 1:4, sep="_"))
               # paste("d28", rep("EC",4), paste(1:4, ".", sep="", 2), sep="_"))

txi <- tximport(filesEC, type="salmon", txOut=TRUE) 
...
# head(tx2gene_mus) 
txigene <- tximport(filesEC, type="salmon", tx2gene=tx2gene_mus)


samples_EC$timepoint <- as.factor(samples_EC$timepoint)
...
# ggsave("SampleDistances_EC_genelevel.png", plot = p)
...
pca <- ggplot(pcaData, aes(PC1, PC2, shape=timepoint, color=rep)) +
  ...
  coord_fixed() +
  ggtitle("EC: PCA with VST data (on gene level)")

ggsave("PCAplot_EC_genelevel.pdf", plot = pca)
...
write.table(normalized_counts, file="data/normalized_genecounts_EC.txt", sep="\t", quote=F, col.names=NA)
# write.table(normalized_counts, file="data/normalized_counts_EC.txt", sep="\t", quote=F, col.names=NA)
...
#       c_addsn = c(); c_addsno = c(); c_addr = c(); c_addmi = c()
...
# ggsave("Violinplot_EC_d0.png", plot = violin_d0)
...
# ggsave("Violinplot_EC_d1.png", plot = violin_d1)
...
# ggsave("Violinplot_EC_d3.png", plot = violin_d3)
...
# ggsave("Violinplot_EC_d7.png", plot = violin_d7)
...
# ggsave("Violinplot_EC_d14.png", plot = violin_d14)
